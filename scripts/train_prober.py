...
def main(args):
    ...
    train_Y_original = torch.load(f"outputs/activations/{model_name}/{concept}/train/labels.pt")
    test_Y_original = torch.load(f"outputs/activations/{model_name}/{concept}/test/labels.pt")
    ...
    index_layers = list(range(0, llm_num_layers))
    pbar = tqdm(index_layers, desc=f"{config.save_dir}")
    for target_layer in pbar:
        
        train_X_original = torch.load(f"outputs/activations/{model_name}/{concept}/train/layer_{target_layer}_key.pt")
        test_X_original = torch.load(f"outputs/activations/{model_name}/{concept}/test/layer_{target_layer}_key.pt")
        ...
        print("Test Accuracy", f"{test_accs[-1]:.4f}")
